...
%g
%g\paragraph{次密钥加密}
%g    有如下元素 \begin{enumerate}
%g        \item 文字$M = \langle M_0, \ldots M_9 \rangle$
%g        \item 密钥$K = \langle K_0, \ldots K_9 \rangle$
%g        \item $\sigma_i$是已知的8位置换
%g        \item $\sigma_{SW}$是已知的8位置换
%g        \item $f_{K_i}$是8位到8位的函数, 由次密钥$K_i$参数化
%g        \item $\sigma{LS, l}$是一个已知的10位置换
%g    \end{enumerate}
%g    则有 \begin{align*}
%g        D(M) &= \sigma_i^{-1} ( f_{K_2} ( \sigma_{SW} ( f_{K_1} ( \sigma_i (M) ) ) ) ) \\
%g        E(C) &= \sigma_i^{-1} ( f_{K_1} ( \sigma_{SW} ( f_{K_2} ( \sigma_i (C) ) ) ) )
%g    \end{align*}
%g    其中$f_{K_i}(X)$的定义如下.\par
%g    有如下元素 \begin{enumerate}
%g        \item $EP$是已知的4位到8位的函数
%g        \item $S0, S1$是已知的4位到2位的函数
%g        \item $\sigma_4$是4位置换
%g    \end{enumerate}
%g    则算法如下 (以下特别使用大段顺序) \begin{enumerate}
%g        \item $t = EP(X_{3 \ldots 0}) \oplus K_i$
%g        \item $h = X_{7 \ldots 4} \oplus \sigma_4(S0(t_{7 \ldots 4}) , S1(t_{3 \ldots 0}))$
%g        \item $f_{K_i}(X) = (h, X_{3 \ldots 0})$
%g    \end{enumerate}
%g\paragraph{计算$K_1, K_2$}
%g    有如下元素 \begin{enumerate}
%g    \end{enumerate}
