...
\paragraph{去除直接左递归} 原为\begin{align*}
    ...
    \end{align*}
\paragraph{去除间接左递归} 要求原文法无环$A \not\Rightarrow^+ A$, 无$\epsilon$产生式$A \to \epsilon$. 按照如下算法\begin{enumerate}
        \item 将非终结符编号为$A_1, A_2 \ldots A_n$
        \item \begin{verbatim}for i = 1 to n
    for j = 1 to i-1
        对于所有 A_i -> A_j \alpha, 代替以 A_i -> \beta \alpha, 其中 A_j -> \beta
    消除A_i的直接递归
\end{verbatim}
    \end{enumerate}
